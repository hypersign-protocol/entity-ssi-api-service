...
export class DidService {
  ...
  async checkAllowence(address) {
    const start = this.logStart(
      'checkAllowence',
      'check wallet feegrants for the provided address',
    );
    try {
      const url =
  ...
  async createByClientSpec(createDidDto: CreateDidDto, appDetail) {
    const start = this.logStart(
      'createByClientSpec',
      'create a DID from a client-provided spec and wallet info',
    );
    try {
      ...
      this.logEnd('createByClientSpec', start);
    }
  }
  ...
  async create(
    ...
    const start = this.logStart(
      'create',
      'generate a new DID and store its key material in the app vault',
    );
  ...
  async createBjjDid(
    ...
    const start = this.logStart(
      'createBjjDid',
      'generate a new BabyJubJub DID and persist its vault credentials',
    );
    try {
      const methodSpecificId = createDidDto.methodSpecificId;
      // let verificationRelationships: IVerificationRelationships[];
      ...
      this.logEnd('createBjjDid', start);
    }
  }
  ...
  async register(
    ...
    const start = this.logStart(
      'register',
      'register a DID on chain using provided sign info or local wallet data',
    );
    try {
      let registerDidDoc;
      const { edvId, kmsId } = appDetail;
  ...
  async registerV2(
    ...
    const start = this.logStart(
      'registerV2',
      'register a DID with V2 sign infos and optional chain transaction handling',
    );
    try {
      const { edvId, kmsId } = appDetail;
      ...
      for (let i = 0; i < didDocument.verificationMethod.length; i++) {
        ...
        if (keyType === IKeyType.Ed25519VerificationKey2020) {
          ...
          const { mnemonic: userMnemonic } =
            await appVault.getDecryptedDocument(didInfo.kmsId);
  ...
  async getDidList(appDetail, option): Promise<IDidDto[]> {
    const start = this.logStart(
      'getDidList',
      'fetch paginated DID list for the application',
    );
    try {
  ...
  async resolveDid(appDetail, did: string) {
    const start = this.logStart(
      'resolveDid',
      'resolve a DID either from blockchain or fallback local vault',
    );
    try {
      const didInfo = await this.didRepositiory.findOne({
        did,
      ...
      if (
        ...
        const { edvId, kmsId } = appDetail;
        Logger.log(
          'resolveDid() method: initialising edv service',
          'DidService',
        );
  ...
  async updateDid(
    ...
    const start = this.logStart(
      'updateDid',
      'update or deactivate a DID document using Hypersign operations',
    );
    try {
      let updatedDid;
  ...
  async addVerificationMethod(
    ...
    const start = this.logStart(
      ...
      'append a verification method to an existing DID document',
    );
    try {
      const hypersignDid = new HypersignDID();
      let result;
      ...
      this.logEnd('addVerificationMethod', start);
    }
  }
  ...
  async SignDidDocument(signDidDto: SignDidDto, appDetail) {
    const start = this.logStart(
      'SignDidDocument',
      'sign a DID document using the app vault private key',
    );
    try {
      const { edvId, kmsId } = appDetail;
      const appMenemonic = await getAppMenemonic(kmsId);
       ...
       */
      if (
        !signDidDto.didDocument &&
        DidInfo.registrationStatus != 'COMPLETED'
      ) {
        throw new BadRequestException([
      ...
      this.logEnd('SignDidDocument', start);
    }
  }
  async VerifyDidDocument(verifyDidDto: VerifyDidDto, appDetail) {
    const start = this.logStart(
      'VerifyDidDocument',
      'verify the proof on a signed DID document',
    );
    try {
      ...
      this.logEnd('VerifyDidDocument', start);
    }
  }
  async issueDidJwt(issueDidJwtDto: IssueDidJwtDto, appDetail) {
    const start = this.logStart(
      'issueDidJwt',
      'issue a DID-based JWT for the given issuer and claims',
    );
    try {
      const { edvId, kmsId } = appDetail;
